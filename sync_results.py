...
MAINLINE_SPECS: list[ResultSpec] = [
    ResultSpec(
        ...
        summary_path="outputs/training/models/dyrift_gnn/full_xinye_repro_v1/summary.json",
    ...
    ResultSpec(
        ...
        summary_path=(
            "outputs/elliptic_transactions/training/models/dyrift_gnn/"
    ...
    ResultSpec(
        ...
        summary_path=(
            "outputs/ellipticpp_transactions/training/models/dyrift_gnn/"
...
COMPARISON_SPECS: list[ResultSpec] = [
    *tri_specs(
        ...
        path_template="outputs/studies/comparisons/plain_trgt_backbone/{dataset}/summary.json",
    ),
    *tri_specs(
        group="comparisons",
        study="tgat_style_reference",
        setting="TGAT-style Reference",
        path_template="outputs/studies/comparisons/tgat_style_reference/{dataset}/summary.json",
    ),
    *tri_specs(
        group="comparisons",
        study="temporal_graphsage_reference",
        setting="Temporal GraphSAGE Reference",
        path_template="outputs/studies/comparisons/temporal_graphsage_reference/{dataset}/summary.json",
    ),
    *tri_specs(
        group="comparisons",
        study="xgboost_same_input",
        setting="XGBoost Same Input",
        path_template="outputs/studies/comparisons/xgboost_same_input/{dataset}/summary.json",
    ),
]
...
ABLATION_SPECS: list[ResultSpec] = [
    *tri_specs(
        ...
        path_template="outputs/studies/ablations/without_target_context_bridge/{dataset}/summary.json",
    ),
    *tri_specs(
        group="ablations",
        study="without_drift_expert",
        setting="w/o Drift Expert",
        path_template="outputs/studies/ablations/without_drift_expert/{dataset}/summary.json",
    ),
    *tri_specs(
        group="ablations",
        study="without_prototype_memory",
        setting="w/o Prototype Memory",
        path_template="outputs/studies/ablations/without_prototype_memory/{dataset}/summary.json",
    ),
    *tri_specs(
        group="ablations",
        study="without_pseudo_contrastive",
        setting="w/o Pseudo-Contrastive Temporal Mining",
        path_template="outputs/studies/ablations/without_pseudo_contrastive/{dataset}/summary.json",
    ),
]
...
PROGRESSIVE_SPECS: list[ResultSpec] = [
    *tri_specs(
        ...
        study="trgt_bridge",
        setting="TRGT + Bridge",
        path_template="outputs/studies/progressive/trgt_bridge/{dataset}/summary.json",
    ),
    *tri_specs(
        group="progressive",
        study="trgt_bridge_drift",
        setting="TRGT + Bridge + Drift Expert",
        path_template="outputs/studies/progressive/trgt_bridge_drift/{dataset}/summary.json",
    ),
    *tri_specs(
        group="progressive",
        study="trgt_bridge_drift_prototype",
        setting="TRGT + Bridge + Drift Expert + Prototype Memory",
        path_template="outputs/studies/progressive/trgt_bridge_drift_prototype/{dataset}/summary.json",
    ),
    *tri_specs(
        group="progressive",
...
def build_accepted_mainline_summary(
    ...
    return {
        ...
        "maintained_rerun_policy": {
            ...
            "policy_path": "configs/training_policy.json",
...
def build_epoch_manifest_rows(
    ...
    for spec in ALL_RESULT_SPECS:
        ...
        epoch_metrics_path = artifact_path_value(seed_metric.get("epoch_metrics_path"))
        rows.append(
            {
                ...
                "train_log_path": artifact_path_value(seed_metric.get("train_log_path")),
                "curve_path": artifact_path_value(seed_metric.get("curve_path")),
                "summary_path": spec.summary_path,
            }
        )
    return rows
...
def build_training_policy_summary() -> dict[str, Any]:
    policy_path = REPO_ROOT / "configs" / "training_policy.json"
    ...
    return {
        ...
        "machine_readable_config": "configs/training_policy.json",
...
def validate_policy_files() -> None:
    policy = json.loads(
        (REPO_ROOT / "configs" / "training_policy.json").read_text(
            encoding="utf-8"
        )
    )
...
def artifact_path_value(value: Any) -> str:
    path = string_value(value).strip()
    if path.startswith("./"):
        path = path[2:]
    legacy_prefix = "experiment" + "/"
