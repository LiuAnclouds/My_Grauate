...
from experiment.models.presets import _BASE_GRAPH_CONFIG, apply_cfg_overrides, build_graph_cfg
...
def run_graph_dataset(
    *,
    ...
    with tqdm(
        seeds,
        desc=f"study:{study.study_name}:{plan.dataset_short}",
        unit="seed",
        dynamic_ncols=True,
    ) as seed_pbar:
        for seed in seed_pbar:
            ...
            _write_enriched_fit_summary(
                path=seed_dir / "fit_summary.json",
            ...
            train_prob = experiment.predict_proba(
                ...
                progress_desc=f"{study.study_name}:seed{seed}:train",
            )
            val_prob = experiment.predict_proba(
                phase1_context,
                val_ids,
                batch_seed=int(seed) + 1000,
                progress_desc=f"{study.study_name}:seed{seed}:val",
            )
            test_pool_prob = (
                experiment.predict_proba(
                    ...
                    progress_desc=f"{study.study_name}:seed{seed}:test_pool",
                )
            ...
            external_prob = (
                experiment.predict_proba(
                    ...
                    progress_desc=f"{study.study_name}:seed{seed}:external",
                )
                if external_ids.size
                else None
            )
            ...
            tqdm.write(
                f"[study:{study.study_name}] dataset={plan.dataset_name} seed={seed} "
    ...
    summary = {
        ...
        "graph_config_overrides": list(plan.graph_config_overrides) + list(graph_spec.get("graph_config_overrides", [])),
        ...
        "no_leakage_notes": [
            ...
            "Validation and test_pool labels are evaluation-only and do not enter optimization.",
            "Each dataset is trained in an isolated process with its own dataset-scoped caches.",
            "This study reuses the production split contract and does not alter train/val boundaries.",
        ],
    }
...
def _write_enriched_fit_summary(path: Path, payload: dict[str, Any]) -> None:
