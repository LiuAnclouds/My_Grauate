...
当前推荐主线固定为一条统一的 **deployable pure-GNN DyRIFT-GNN**：
...
训练入口支持一份 manifest 加三个独立 JSON 管理数据集级超参数：
...
- `DyRIFTModel`: final DyRIFT-GNN model facade
- `build_model`: final model factory used by the trainer
- `DyRIFTTrainer`: final training wrapper for `dyrift_gnn`
